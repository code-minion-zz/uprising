...
public class QuickStart2 : EditorWindow
{
	...
	private static void InstallProBuilder()
	{
		...
		needsOrganized = probuilderExists && (
			!pbcore_path.Replace("\\", "/").Contains("Assets/ProCore/ProBuilder/Classes") ||
			!pbeditor_path.Replace("\\", "/").Contains("Assets/ProCore/ProBuilder/Editor") );
		...
		if(probuilderExists)
		{
			...
			if(!needsOrganized)
			{
				...
				ImportPack( (InstallType)(pbcore_path.Contains(".dll") ? 0 : 1) );
